...
SENPURE.ORDER_NONE = 0;
SENPURE.ORDER_DESC = 1;
SENPURE.ORDER_ASC = 2;
...
$(function() {
	...
	$('.table-responsive').each(function(index, element) {
				...
				if (total) {
					...
					fenye.excelWaitClose = jb.attr('data-excel-wati-close');
	...
	if ($('form[auto-refresh=true]').length > 0) {
		...
		$('form[auto-refresh=true]').each(function(index, e) {
					...
					var $e = $(e);
					var time = $e.attr('refresh-interval') || SENPURE.minutes5;
					setInterval(function() {
								checkRefresh($e, time)
	...
	$('th[data-order=true] , td[data-order=true]').each(function(index, e) {
		...
		$e.addClass("cursor-pointer").click(function() {
					...
					$(fid + ' input[name=firstResult]').val(0);
					$(fid).submit();
				});
...
function sortMenu(one, two) {
	if (one.parentId == two.parentId) {
		...
		return one.sort < two.sort ? -1 : one.sort > two.sort ? 1 : 0;
	}
	return 0;
}
...
function generatorPagination(data) {
	// console.debug('生成分页');
	var tfirst = parseInt(data.first) + 1;
	// console.debug('cpage data.first=' + data.first + ' ,data.max=' + data.max
	// + ' , tfirst=' + tfirst + ' ,' + tfirst / data.max);

	var cpage = Math.floor(tfirst / data.max) + 1;

	// console.debug('cpage ' + cpage);
	var firstpage = cpage == 1 ? true : false;

	var page = Math.floor(data.total / data.max) + 1;
	...
	html += '</span></li>';
	if (!firstpage) {
		html += '<li><a href="" onclick="paginationClick(&#34;' + data.form
				+ '&#34;,' + 0 + ',' + data.max + ');return false;" >'
				+ data.firstText + '</a></li>';
		html += '<li><a href="" onclick="paginationClick(&#34;' + data.form
				+ '&#34;,' + (cpage - 2) * data.max + ',' + data.max
				+ ');return false;" >'
	...
	for (var i = start; i <= end; i++) {
		...
		html += '>'
		html += '<a href="" onclick="paginationClick(&#34;' + data.form
				+ '&#34;,' + data.max * (i - 1) + ',' + data.max
				+ ');return false;">' + i + '</a> </li>';
	}

	if (!lastpage) {

		html += '<li><a href="" onclick="paginationClick(&#34;'
				+ data.form
				+ '&#34;,'
				+ data.max
				* cpage
		...
		html += '<li><a href=""  onclick="paginationClick(&#34;' + data.form
				+ '&#34;,' + data.max * (page - 1) + ',' + data.max
				+ ');;return false;">' + data.lastText + '</a></li>';
	}
	if (data.excelText) {

		html += '<li><a href="' + currentPath
...
function paginationClick(formId, start, max) {
	// console.log('click----' + start + ',max' + max)
	// console.log($(formId + ' input[name=firstResult]').val());
	$(formId + ' input[name=firstResult]').val(start);
	// console.log($(formId + ' input[name=firstResult]').val());
	$(formId + ' input[name=maxResults]').val(max);
	$(formId).submit();
